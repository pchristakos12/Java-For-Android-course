...
public class Logic
    ...
    public double calculate(int size, int count) {
        ...
        for(int i = 0 ; i<count;i++){
            int possibleBdays[] = new int[365];
            rand.setSeed(i+1);
            for(int j = 0;j<size;j++){
                int randomBdays = rand.nextInt(365);
                possibleBdays[randomBdays]++;
                if(possibleBdays[randomBdays]>=2){
                    birthdayCounter++;
                    break;
                }
        ...
        return birthdayCounter*100.0/count;
